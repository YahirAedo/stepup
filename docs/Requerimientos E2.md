...
| Idempotencia por `Idempotency-Key` | Los writes requieren key idempotente (patrón Stripe) para evitar duplicados en reintentos. Implementado del lado servidor y del cliente (PR #175, DT-26): la key se persiste por operación (`pending_idempotency_keys`) y se reutiliza en reintentos tras fallo de red; se limpia al confirmar éxito. | Server ✅ / cliente ✅ (PR #175) |
