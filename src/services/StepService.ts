...
export const StepService = {
  ...
  async update(id: number, input: UpdateStepInput): Promise<void> {
    ...
    if (input.duration_min !== undefined) {
      ...
      values.push(input.duration_min);
    }
    if (fields.length === 0) return;
    values.push(id);
    await db.runAsync(`UPDATE steps SET ${fields.join(', ')} WHERE id = ?`, values);
  },
  ...
  async reorder(task_id: number, orderedIds: number[]): Promise<void> {
    ...
    await db.withTransactionAsync(async () => {
      for (let i = 0; i < orderedIds.length; i++) {
        await db.runAsync(`UPDATE steps SET order_index = ? WHERE id = ? AND task_id = ?`, [
          i,
          orderedIds[i],
          task_id,
        ]);
