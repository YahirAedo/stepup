...
export function handleError(res: Response, error: unknown) {
  ...
  if (error instanceof AiRateLimitError) {
    return res
      .status(429)
      .json({ message: 'El servicio de IA está saturado en este momento. Intenta nuevamente en unos minutos.' });
  }

  if (error instanceof AiProviderError) {
    return res
      .status(502)
      .json({ message: 'El servicio de IA no está disponible en este momento. Intenta nuevamente en unos minutos.' });
  }
